...
import 'confirm_credit_sale_popup.dart';
...
class _SalesInvoiceScreenState extends State<SalesInvoiceScreen> {
  ...
  Future<void> _addSaleAPIDirect(BuildContext context) async {
    ...
    try {
      ...
      List<SalesDetail> salesDetails = items.map((item) {
        return SalesDetail(
          itemId: item.itemId,
          unitId: item.unitId,
  ...
  void _onDiscountChanged() {
    ...
    // ✅ TAX AFTER DISCOUNT (5%)
    double newTaxAmt = newTaxableAmt * 0.05;
  ...
  void _recalculateTotals() {
    ...
    // ✅ CAPTURE TAX RATE HERE (correct place)
    if (taxableAmt > 0) {
...
//check expiry date
  ...
  Future<void> _addNewItemToExistingSale(String? billType, int? locationId,
      String? payMode, String taxType) async {
    try {
      ...
      if (newItem != null) {
        ...
        final exists = items.any((item) => item.itemId == newItem.itemId);
                                ...
                                if (result != null &&
                                  ...
                                  setState(() {
                                    customerController.text =
                                        result['customer'] ?? "";
                                    selectedCustId = result['customerId'] ?? 0;
                                    address = result['address'] ?? "-";
                                    byHand = result['byHand'] ?? "-";
                                    contact = result['contact'] ?? "-";
                                    selectedCreditLimit =
                                        result['creditLimit']?.toString() ??
                                            "0";
                                    selectedOpeningBal =
                                        result['openingBal']?.toString() ?? "0";
                                  });
                                }
                              },
                              child: Container(
                              ...
                              onTap: () async {
                                ...
                                if (result != null &&
                                  ...
                                  setState(() {
                                    ...
                                    selectedTaxType = result['taxType'] ?? "-";
                                    selectedBillType =
                                        result['billType'] ?? "-";
                                    selectedLocationId =
                                        result['locationId'] ?? "-";
                                    selectedPayMode =
                                        result['payMode']?.toString() ?? "-";
                                  });
                                }
                              },
                              child: Container(
                                  ...
                                  : () async {
                                      ...
                                      if (isEdit) {
                                        ...
                                        if (invoiceItem != null) {
                                          ...
                                          //           Colors.red.shade600,
                                          //       content: Text(
                                          //         "Only ${invoiceItem.stock} qty available in stock",
                                          //       ),
                                          //     ),
                                          //   );
                                          //   return;
                                            ...
                                            ScaffoldMessenger.of(context)
                                              ...
                                              SnackBar(
                                                backgroundColor:
                                                    Colors.red.shade600,
                                                content: Text(
                                                  "Free quantity (${invoiceItem.freeQty}) cannot exceed available stock (${invoiceItem.stock}).",
                                                ),
                                              ),
                                            );
                                            return;
                                                  ...
                                                  child: SingleChildScrollView(
                                                    child: Column(
                                                      ...
                                                      children: [
                                                        ...
                                                        // _buildDetailRow(
                                                        //     Strings
                                                        //         .dashboardProductRow3,
                                                        //     product.batch),
                                                        // _buildDetailRow(
                                                        //     Strings
                                                        //         .dashboardProductRow4,
                                                        //     product.expiry !=
                                                        //             null
                                                        //         ? _formatDate(
                                                        //             product
                          ...
                          child: ElevatedButton(
                            ...
                            onPressed: () async {
                              ...
                              if (customerController.text.trim().isEmpty ||
                                ...
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(
                                    backgroundColor: Colors.red.shade600,
                                    content:
                                        Text(Strings.selectCustomerSnackText),
                                  ),
                                );
                                return;
                              }
                              ...
                              double roundOffValue =
                                  double.tryParse(roundOffController.text) ??
                                      0.0;
                              ...
                              print('Selected pay mode: $payMode');
                              print('Selected tax type mode: $selectedTaxType');

                              String taxTypeCode = "N";

                              if (selectedTaxType?.toLowerCase() ==
                                  "inclusive") {
                                taxTypeCode = "I";
                              } else if (selectedTaxType?.toLowerCase() ==
                                  "exclusive") {
                                taxTypeCode = "E";
                              } else {
                                taxTypeCode = "N";
                              }
                              if (payMode?.trim().toLowerCase() == "credit") {
                                print("Calling credit API directly...");
                                // 🔹 Call Add Sale API directly
                                await _addSaleAPIDirect(context);
                              } else {
                                await showDialog(
                                    context: context,
                                    builder: (_) => ConfirmCashPaymentDialog(
                                          // isEdit: widget.isEdit,
                                          salesId: sale?.salesID ?? 0,
                                          salesDate: DateFormat('yyyy-MM-dd')
                                              .format(salesDate),
                                          ...
                                          freeQty: freeQty,
                                          paymentTerm:
                                              selectedPaymentTerm.toString(),
                                          items: items,
                                          otherCharges: freightCharges,
                                          customerId: selectedCustId,
                                          customerName: customerController.text,
                                          address: address,
                                          invoiceChargesAmt: freightValue,
