...
class PurchaseDashboard extends StatefulWidget {
  ...
  @override
  _PurchaseDashboardScreenState createState() =>
      _PurchaseDashboardScreenState();
}

class _PurchaseDashboardScreenState extends State<PurchaseDashboard> {
  DateTime? startDate;
  DateTime? endDate;

  final TextEditingController searchController = TextEditingController();
  List<PurchaseDashboardModel> filteredPurchases = [];
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) {
        _fetchPurchases(); // fetch default on load
      }
    });
  }
  ...
  void _fetchPurchases() async {
    ...
    await purchaseVM.fetchPurchasesByFilter(
      fromDate: fromDate,
      toDate: toDate,
    ...
    setState(() {
      filteredPurchases = List.from(purchaseVM.purchaseDashboardList);
    });
  }
  ...
  void _filterSearchResults(String query) {
    ...
    if (query.isEmpty) {
      setState(() {
        filteredPurchases = purchaseVM.purchaseDashboardList;
      });
      return;
    }

    setState(() {
      filteredPurchases = purchaseVM.purchaseDashboardList.where((purchase) {
        ...
        return AlertDialog(
          ...
          actions: [
            ...
            ElevatedButton(
              ...
              onPressed: () {
                Navigator.pop(context);
                _fetchPurchases(); // 👈 call API with selected range
              },
            ),
          ],
        );
  ...
  Widget build(BuildContext context) {
    ...
    final list = searchController.text.isEmpty
        ? purchaseVM.purchaseDashboardList
        : purchaseVM.purchaseDashboardList.where((purchase) {
      final supplierName = purchase.supplierName?.toLowerCase() ?? '';
      final purchaseId = purchase.purchaseId?.toString() ?? '';
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              // 🔹 Changes: Search Bar visibility check added below
    ...
    color: primary,
    onRefresh: () async {
     _fetchPurchases(); // 🔥 refresh API
    },
    child: purchaseVM.loading
    ? const Center(child: GifLoader())
        : list.isEmpty
    ? ListView( // ⚠️ IMPORTANT (for pull even when empty)
    children: const [
    SizedBox(height: 200),
    Center(child: NoDataIcon(size: 70)),
    ],
    )
        : ListView.builder(
