...
class PurchaseChallanScreen extends StatefulWidget {
  final bool isReturn;   // 👈 ADD

  const PurchaseChallanScreen({super.key,
    this.isReturn = false,
  });

  @override
  State<PurchaseChallanScreen> createState() => _PurchaseInwardScreenState();
}

class _PurchaseInwardScreenState extends State<PurchaseChallanScreen> {
  DateTime challanDate = DateTime.now(); // default today
...
//check expiry date
  ...
  void _applyGlobalDiscount() {
    double globalDiscPer =
        double.tryParse(discountController.text) ?? 0;

    for (int i = 0; i < items.length; i++) {
      final item = items[i];

      // ✅ ALWAYS use original values
      double baseTotal = item.originalItemTotal;
      double originalItemDisc = item.originalDiscountAmt;

      // Global discount
      double globalDiscAmt = baseTotal * globalDiscPer / 100;

      // Total discount
      double totalDiscAmt = globalDiscAmt + originalItemDisc;

      // Taxable
      double taxableAmt = baseTotal - totalDiscAmt;

      double taxAmt = taxableAmt * item.taxPerc / 100;
  ...
  Map<String, dynamic> _buildPurchaseRequest() {
    ...
    final purchase = AddPurchaseModel(
      // purchaseId: 0,
      // purchaseSeriesId: 1,
      purchaseDate: DateFormat('yyyy-MM-dd').format(challanDate),
      challanDate: DateFormat('yyyy-MM-dd').format(challanDate),
                              ...
                              child: GestureDetector(
                                onTap: (selectedSupplier == null || selectedLocationId == null)
                                    ...
                                    : () async {
                                        ...
                                        final ledgerBal =
                                            purchaseInvoiceVM.ledgerBalance;
                                        ...
                                        final invoiceItem = await showDialog<
                                          ...
                                          builder: (dialogContext) {
                                            return PurchaseSelectProductPopup(
                                              mode: PurchaseMode.purchase,   // ✅ FIX

                                              payMode: selectedPayMode ?? "CASH",
                                              taxType: taxType,
                                              locationId: selectedLocationId ?? "",

                                              partyId: 0,   // ✅ safe (not used in purchase mode)
                                            );
                                          },
                                        );
...

                              {
                                      ...
                                      if (success) {
                                        ...
                                        ScaffoldMessenger.of(context).showSnackBar(
                                          SnackBar(
                                            content: Text(
                                              Strings.purchaseChallanSaveSuccess,
                                            ),
                                        ...
                                        Navigator.pushAndRemoveUntil(
                                          context,
                                          MaterialPageRoute(
                                            builder: (context) => const PurchaseChallanDashboard(),
                                          ),
                                              (route) => false,
                                        );
                                      } else {
                                        ...
                                        ScaffoldMessenger.of(context).showSnackBar(
                                          const SnackBar(
                                            content: Text(Strings.purchaseChallanSaveFailed),
                                            backgroundColor: Colors.red,
                                          ),
                                        );
